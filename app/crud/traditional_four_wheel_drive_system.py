...
def get_traditional_four_wheel_drive_system_module(db: Session):
    result: [models.TraditionalFourWheelDriveSystemModule] = db.query(
        models.TraditionalFourWheelDriveSystemModule).all()
    return result


def get_traditional_four_wheel_drive_system_module_parameters(db: Session):
    table_module_obj = models.TraditionalFourWheelDriveSystemModule
    table_module_parameters_obj = models.TraditionalFourWheelDriveSystemModuleParameters
    ...
    for module_once in module_data:
        # 通用这里的参数需要修改
        if module_once.traditional_four_wheel_drive_system_module_parameters_id_list:
            res = db.query(table_module_parameters_obj).filter(
                table_module_parameters_obj.id.in_(
                    module_once.traditional_four_wheel_drive_system_module_parameters_id_list)
            ).all()
            # 将查询结果添加到字典中
...
def get_module_parameters_traditional_four_wheel_drive(item: schemas.TraditionalFourWheelDriveSystemDataId,
                                                       db: Session):
    # 初始化结果字典
    ...
    table_son_obj = models.TraditionalFourWheelDriveSystemModuleParameters
    module_data_list = db.query(table_obj).filter(
        table_obj.id.in_(module_data_id_list)
    ).all()
    for module_data_once in module_data_list:
        if module_data_once.traditional_four_wheel_drive_system_module_parameters_id_list:
            # 根据 kc_parameters_id_list 查询对应的 KCParameters
            res = db.query(table_son_obj).filter(
                table_son_obj.id.in_(module_data_once.traditional_four_wheel_drive_system_module_parameters_id_list)
            ).all()
            # 将查询结果添加到字典中
...
def get_traditional_four_wheel_drive_system_data(item: schemas.TraditionalFourWheelDriveSystemData, db: Session):
    result = {}
    all_table = {
